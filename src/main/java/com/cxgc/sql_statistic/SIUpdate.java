...
public class SIUpdate {
    public static void main(String[] args) {
        ...
        try {
            List<DeviceStaticInformation> dsiList = new DSIDao().findAll();
            for (int i = 0; i < dsiList.size(); i++) {
                String ID = dsiList.get(i).getIotDeviceId();
                ...
                addingDate = strToDate("2018-10-4");

                java.util.Date endDate = strToDate("2018-10-19");
                ...
                liesUsingRate += liesUsingRate2;*/
                    ...
                    try {
                        List<StaticInformation> siList = new SIDao().findByIdAndParticularDate(ID
                                , new java.sql.Date(addingDate.getTime()));

                        if (siList.isEmpty()) {
                            new SIDao().add(new StaticInformation(ID,
                            ...
                            new SIDao().updateDailyDistance(ID, new java.sql.Date(addingDate.getTime()), liesDistance);
                            new SIDao().updateDailyUsingRate(ID, new java.sql.Date(addingDate.getTime()), liesUsingRate);
                            new SIDao().updateDailyFuelCost(ID, new java.sql.Date(addingDate.getTime()), liesFuel);
                            new SIDao().updateDailyRunTime(ID, new java.sql.Date(addingDate.getTime()), liesTime);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
